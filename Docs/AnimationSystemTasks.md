...
- [ ] **AnimationClip アセットの実装**
    - [ ] C++: `NodeAnimation` を束ねた `AnimationClip` 構造体の定義 ( `Engine/Asset/Assets/Mesh/Skinning.h` 周辺)
    - [ ] C++: `Model` ロード時にアニメーションデータを `AnimationClip` として抽出・保持するよう修正
    - [ ] C++: `AnimationClip` 内へのイベントデータ保持構造の追加
- [ ] **Animator コンポーネントの実装**
    - [ ] C++: `Animator` コンポーネントクラスの新規作成 (`Engine/ECS/Component/Components/ComputeComponents/Animator/Animator.h`)
    - [ ] C++: **重要：クラスメンバとしてフラットな固定長配列によるレイヤー/ステート管理を実装 (DoD最適化)**
...
- [ ] **AnimatorUpdateSystem の実装**
    - [ ] C++: `AnimatorUpdateSystem` の新規作成。`SkinMeshRenderer` の `Skeleton` に対してジョイントトランスフォームを書き込む
- [ ] **基本 API とデータ連携**
...
- [ ] **AnimationState の導入**
    - [ ] C++: `Animator` 内部に固定長配列によるステート管理を実装
- [ ] **ブレンディングロジックの実装**
    - [ ] C++: `AnimatorUpdateSystem` で `AnimationState` をサンプリングし、補間する
    - [ ] C++: **重要：ブレンド後の Quaternion 正規化 (Normalize) 処理の追加 (歪み防止)**
...
- [ ] **AnimationLayer の実装**
    - [ ] C++: `Animator` が複数の再生レイヤーを持てるように拡張
- [ ] **BoneMask の実装**
    - [ ] C++: `BoneMask` データ構造と JSON ロード対応
- [ ] **レイヤー合成ロジックの実装**
    - [ ] C++: レイヤーごとの BoneMask 重みを適用した合成処理
    - [ ] C++: **合成後の最終ポーズに対する Quaternion 正規化の徹底**
